...
def cpu_intensive_task(task_id):
    ...
    result = fibonacci(n)
    end_time = time.time()
    print(f"Task {task_id}: Fibonacci({n}) = {result}, Time: {end_time - start_time:.2f}s")
...
def run_parallel_tests():
    """Main function to run all parallelism tests"""
    print("=" * 50)
    print("JENKINS PARALLELISM TEST SUITE")
    print("=" * 50)
    
    # Test 1: Threading for I/O bound tasks
    threading_results = test_threading_parallelism()
    print(f"Threading results: {len(threading_results)} tasks completed\n")
    
    # Test 2: Multiprocessing for CPU bound tasks
    multiprocessing_results = test_multiprocessing_parallelism()
    print(f"Multiprocessing results: {len(multiprocessing_results)} tasks completed\n")
    
    # Test 3: Sequential execution for comparison
    sequential_results = test_sequential_execution()
    print(f"Sequential results: {len(sequential_results)} tasks completed\n")
